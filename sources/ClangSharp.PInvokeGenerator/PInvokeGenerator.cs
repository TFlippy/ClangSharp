...
namespace ClangSharp
{
	public sealed partial class PInvokeGenerator: IDisposable
	{
		...
		public PInvokeGenerator(PInvokeGeneratorConfiguration config, Func<string, Stream> outputStreamFactory = null)
		{
			...
			this._index = CXIndex.Create();
			this._outputBuilderFactory = new OutputBuilderFactory();
			this._outputStreamFactory = outputStreamFactory ?? ((path) =>
			{
				var directoryPath = Path.GetDirectoryName(path);
				Directory.CreateDirectory(directoryPath);
				return new FileStream(path, FileMode.Create);
			});
			this._fileContentsBuilder = new StringBuilder();
			this._visitedFiles = new HashSet<string>();
			this._diagnostics = new List<Diagnostic>();
			this._context = new LinkedList<Cursor>();
			this._config = config;
			this._uuidsToGenerate = new Dictionary<string, Guid>();
			this._generatedUuids = new HashSet<string>();
		}
		...
		public PInvokeGeneratorConfiguration Config => this._config;

		public Cursor CurrentContext => this._context.Last.Value;

		public IReadOnlyList<Diagnostic> Diagnostics => this._diagnostics;

		public CXIndex IndexHandle => this._index;

		public Cursor PreviousContext => this._context.Last.Previous.Value;
		...
		public void Close()
		{
			...
			foreach (var foundUuid in this._uuidsToGenerate)
			{
				var iidName = foundUuid.Key;

				if (this._generatedUuids.Contains(iidName))
				{
					continue;
				}
				...
				this._outputBuilder.WriteIndented("public static readonly Guid ");
				this._outputBuilder.Write(iidName);
				this._outputBuilder.Write(" = new Guid(");
				this._outputBuilder.Write(iidValue);
			...
			if (!this._config.GenerateMultipleFiles)
			{
				var outputPath = this._config.OutputLocation;
				stream = this._outputStreamFactory(outputPath);
				leaveStreamOpen = true;

				var usingDirectives = Enumerable.Empty<string>();
				var staticUsingDirectives = Enumerable.Empty<string>();

				foreach (var outputBuilder in this._outputBuilderFactory.OutputBuilders)
				{
				...
				if (usingDirectives.Any())
				{
					...
					{
						sw.NewLine = "\n";
						sw.Write(this._config.HeaderText);
			...
			foreach (var outputBuilder in this._outputBuilderFactory.OutputBuilders)
			{
				var outputPath = outputBuilder.IsTestOutput ? this._config.TestOutputLocation : this._config.OutputLocation;

				var isMethodClass = this._config.MethodClassName.Equals(outputBuilder.Name);
				...
				this.CloseOutputBuilder(stream, outputBuilder, isMethodClass, leaveStreamOpen, emitNamespaceDeclaration);
				emitNamespaceDeclaration = false;
			}

			if (leaveStreamOpen && this._outputBuilderFactory.OutputBuilders.Any())
			{
				if (methodClassOutputBuilder != null)
				{
					this.CloseOutputBuilder(stream, methodClassOutputBuilder, isMethodClass: true, leaveStreamOpen, emitNamespaceDeclaration);
				}
			...
			this._outputBuilderFactory.Clear();
			this._visitedFiles.Clear();
		}
		...
		{
			Debug.Assert(this._outputBuilder is null);

			this._filePath = filePath;
			this._clangCommandLineArgs = clangCommandLineArgs;
			...
			if (this._config.GenerateMacroBindings)
			{
				...
				var file = translationUnitHandle.GetFile(this._filePath);
				var fileContents = translationUnitHandle.GetFileContents(file, out var size);
#if NETCOREAPP
				this._fileContentsBuilder.Append(Encoding.UTF8.GetString(fileContents));
#else
				...
				foreach (var cursor in translationUnit.TranslationUnitDecl.CursorChildren)
				{
					...
					{
						this.VisitPreprocessedEntity(preprocessedEntity);
					}
				}

				using var unsavedFile = CXUnsavedFile.Create(this._filePath, this._fileContentsBuilder.ToString());
				var unsavedFiles = new CXUnsavedFile[] { unsavedFile };

				translationFlags = this._translationFlags & ~CXTranslationUnit_Flags.CXTranslationUnit_DetailedPreprocessingRecord;
				var handle = CXTranslationUnit.Parse(this.IndexHandle, this._filePath, this._clangCommandLineArgs, unsavedFiles, translationFlags);

				using var nestedTranslationUnit = TranslationUnit.GetOrCreate(handle);
				this.Visit(nestedTranslationUnit.TranslationUnitDecl);
			}
			else
			{
				this.Visit(translationUnit.TranslationUnitDecl);
			}
		}
		...
		private void AddDiagnostic(DiagnosticLevel level, string message)
		{
			...
			if (this._diagnostics.Contains(diagnostic))
			{
				return;
			}
		...
		private void AddNativeTypeNameAttribute(string nativeTypeName, string prefix = null, string postfix = null, string attributePrefix = null)
		{
			...
			else
			{
				...
				this._outputBuilder.Write(prefix);
			}
			...
			{
				this._outputBuilder.Write(attributePrefix);
			}

			this._outputBuilder.Write("NativeTypeName(\"");
			this._outputBuilder.Write(this.EscapeString(nativeTypeName));
			this._outputBuilder.Write("\")]");
			...
			{
				this._outputBuilder.Write(postfix);
			}
		}
		...
		private void CloseOutputBuilder(Stream stream, OutputBuilder outputBuilder, bool isMethodClass, bool leaveStreamOpen, bool emitNamespaceDeclaration)
		{
			...
			if (this._config.GenerateMultipleFiles)
			{
				if (this._config.HeaderText != string.Empty)
				{
					sw.WriteLine(this._config.HeaderText);
				}
			...
			{
				sw.Write("namespace ");
				sw.Write(this.Config.Namespace);
			...
			if (isMethodClass)
			{
				...
				if (this._isMethodClassUnsafe)
				{
					sw.Write("unsafe ");
				}

				sw.Write("partial class ");
				sw.WriteLine(this.Config.MethodClassName);
				sw.Write(indentationString);
				sw.WriteLine('{');
		...
		{
			if (name.StartsWith(this._config.MethodPrefixToStrip))
			{
				name = name.Substring(this._config.MethodPrefixToStrip.Length);
			}

			return this.EscapeName(name);
		}

		private string EscapeCharacter(char value) => this.EscapeString(value.ToString());
		...
		private string GetAccessSpecifierName(NamedDecl namedDecl)
		{
			...
			switch (namedDecl.Access)
			{
				...
				{
					name = "internal";
					this.AddDiagnostic(DiagnosticLevel.Warning, $"Unknown access specifier: '{namedDecl.Access}'. Falling back to '{name}'.", namedDecl);
					break;
				}
			}
		...
		private Type[] GetBitfieldCount(RecordDecl recordDecl)
		{
			...
			foreach (var fieldDecl in recordDecl.Fields)
			{
				...
				else if (this._config.GenerateUnixTypes && (currentSize > previousSize))
				{
		...
		{
			if (this._config.WithCallConvs.TryGetValue(remappedName, out var callConv) || this._config.WithCallConvs.TryGetValue("*", out callConv))
			{
				return callConv;
			}
			...
			switch (callingConvention)
			{
				...
				{
					var name = "Winapi";
					this.AddDiagnostic(DiagnosticLevel.Warning, $"Unsupported calling convention: '{callingConvention}'. Falling back to '{name}'.", cursor);
					return name;
				}
			}
		}
		...
		private string GetCursorName(NamedDecl namedDecl)
		{
			...
			if (string.IsNullOrWhiteSpace(name))
			{
				if (namedDecl is TypeDecl typeDecl)
				{
					...
					{
						name = this.GetAnonymousName(tagDecl, tagDecl.TypeForDecl.KindSpelling);

					}
					else
					{
						name = this.GetTypeName(namedDecl, context: null, typeDecl.TypeForDecl, out var nativeTypeName);
				...
				{
					name = this.GetAnonymousName(fieldDecl, fieldDecl.CursorKindSpelling);
				}
				else
				{
					this.AddDiagnostic(DiagnosticLevel.Error, $"Unsupported anonymous named declaration: '{namedDecl.Kind}'.", namedDecl);
				}
			}
		...
		private string GetCursorQualifiedName(NamedDecl namedDecl, bool truncateFunctionParameters = false)
		{
			...
			while (parts.Count != 0)
			{
				AppendNamedDecl(part, this.GetCursorName(part), qualifiedName);
		...
		{
			var name = this.GetCursorQualifiedName(namedDecl);

			var remappedName = this.GetRemappedName(name, namedDecl, tryRemapOperatorName: true);
			...
			name = this.GetCursorQualifiedName(namedDecl, truncateFunctionParameters: true);
			remappedName = this.GetRemappedName(name, namedDecl, tryRemapOperatorName: true);
			...
			name = this.GetCursorName(namedDecl);
			remappedName = this.GetRemappedName(name, namedDecl, tryRemapOperatorName: true);
			...
			else if ((namedDecl is RecordDecl recordDecl) && name.StartsWith("__AnonymousRecord_"))
			{
				...
				if (recordDecl.Parent is RecordDecl parentRecordDecl)
				{
					...
					{
						remappedName = "_";
						remappedName += this.GetRemappedCursorName(matchingField);
					}
		...
		{
			if (this._config.RemappedNames.TryGetValue(name, out var remappedName))
			{
				return AddUsingDirectiveIfNeeded(remappedName);
			}

			if (name.StartsWith("const ") && this._config.RemappedNames.TryGetValue(name.Substring(6), out remappedName))
			{
				return AddUsingDirectiveIfNeeded(remappedName);
			}
			...
			if ((cursor is FunctionDecl functionDecl) && tryRemapOperatorName && this.TryRemapOperatorName(ref remappedName, functionDecl))
			{
				return AddUsingDirectiveIfNeeded(remappedName);
			}
			...
			string AddUsingDirectiveIfNeeded(string remappedName)
			{
				...
				{
					this._outputBuilder.AddUsingDirective("System.Numerics");
				}
		...
		{
			var name = this.GetTypeName(cursor, context, type, out nativeTypeName);
			name = this.GetRemappedName(name, cursor, tryRemapOperatorName: false);
			...
			if ((canonicalType is RecordType recordType) && name.Contains("anonymous at")) // hack
			{
				...
				if (recordDecl.Parent is RecordDecl parentRecordDecl)
				{
					...
					{
						name = "_";
						name += this.GetRemappedCursorName(matchingField);
					}
					else if (parentRecordDecl.AnonymousDecls.Count > 1)
					{
			...
			{
				var enumDeclName = this.GetRemappedCursorName(enumDecl);
				...
				this.WithType("*", ref name, ref nativeTypeName);
				this.WithType(enumDeclName, ref name, ref nativeTypeName);
			}
			...
			{
				var enumConstantDeclName = this.GetRemappedCursorName(enumConstantDecl);
		...
		private string GetTypeName(Cursor cursor, Cursor context, Type type, out string nativeTypeName)
		{
			...
			{
				name = this.GetTypeName(cursor, context, arrayType.ElementType, out var nativeElementTypeName);
			...
			{
				name = this.GetTypeName(cursor, context, attributedType.ModifiedType, out var nativeModifiedTypeName);
			}
			else if (type is BuiltinType)
			{
				switch (type.Kind)
				{
					...
					case CXTypeKind.CXType_ULong:
					{
						...
						{
							name = this._config.GeneratePreviewCodeNint ? "nuint" : "UIntPtr";
						}
						else
						{
					...
					case CXTypeKind.CXType_Long:
					{
						if (this._config.GenerateUnixTypes)
						{
							name = this._config.GeneratePreviewCodeNint ? "nint" : "IntPtr";
						}
						else
						{
							goto case CXTypeKind.CXType_Int;
					...
					default:
					{
						this.AddDiagnostic(DiagnosticLevel.Warning, $"Unsupported builtin type: '{type.TypeClass}'. Falling back '{name}'.", cursor);
						break;
					}
				}
			}
			...
			{
				name = this.GetTypeName(cursor, context, deducedType.CanonicalType, out var nativeDeducedTypeName);
			}
			else if (type is ElaboratedType elaboratedType)
			{
				name = this.GetTypeName(cursor, context, elaboratedType.NamedType, out var nativeNamedTypeName);
			}
			else if (type is FunctionType functionType)
			{
				name = this.GetTypeNameForPointeeType(cursor, context, functionType, out var nativeFunctionTypeName);
			}
			else if (type is PointerType pointerType)
			{
				name = this.GetTypeNameForPointeeType(cursor, context, pointerType.PointeeType, out var nativePointeeTypeName);
			}
			else if (type is ReferenceType referenceType)
			{
				name = this.GetTypeNameForPointeeType(cursor, context, referenceType.PointeeType, out var nativePointeeTypeName);
			}
			...
			else if (type is TagType tagType)
			{
				...
				{
					name = this.GetAnonymousName(tagType.Decl, tagType.KindSpelling);
				}
				else if (tagType.Handle.IsConstQualified)
				{
					name = this.GetTypeName(cursor, context, tagType.Decl.TypeForDecl, out var nativeDeclTypeName);
				}
				else
				{
			...
			else if (type is TypedefType typedefType)
			{
				...
				var remappedName = this.GetRemappedName(name, cursor, tryRemapOperatorName: false);

				if (remappedName.Equals(name))
				{
					name = this.GetTypeName(cursor, context, typedefType.Decl.UnderlyingType, out var nativeUnderlyingTypeName);
				}
				else
				{
					name = remappedName;
			...
			{
				this.AddDiagnostic(DiagnosticLevel.Warning, $"Unsupported type: '{type.TypeClass}'. Falling back '{name}'.", cursor);
			}
		...
		private string GetTypeNameForPointeeType(Cursor cursor, Cursor context, Type pointeeType, out string nativePointeeTypeName)
		{
			...
			{
				name = this.GetTypeNameForPointeeType(cursor, context, attributedType.ModifiedType, out var nativeModifiedTypeName);
			}
			else if (pointeeType is FunctionType functionType)
			{
				if (this._config.GeneratePreviewCodeFnptr && (functionType is FunctionProtoType functionProtoType))
				{
					var remappedName = this.GetRemappedName(name, cursor, tryRemapOperatorName: false);
					var callConv = this.GetCallingConventionName(cursor, functionType.CallConv, remappedName, isForFnptr: true);

					var needsReturnFixup = false;
					var returnTypeName = this.GetRemappedTypeName(cursor, context: null, functionType.ReturnType, out _);
					...
					var isMacroDefinitionRecord = (cursor is VarDecl varDecl) && this.GetCursorName(varDecl).StartsWith("ClangSharpMacro_");
					...
					if ((cursor is CXXMethodDecl cxxMethodDecl) && (context is CXXRecordDecl cxxRecordDecl))
					{
						...
						needsReturnFixup = cxxMethodDecl.IsVirtual && this.NeedsReturnFixup(cxxMethodDecl);

						nameBuilder.Append(this.EscapeName(cxxRecordDeclName));
						nameBuilder.Append('*');
						nameBuilder.Append(',');
						nameBuilder.Append(' ');
					...
					if (isMacroDefinitionRecord)
					{
						...
						if (this.IsStmtAsWritten(varDecl.Init, out DeclRefExpr declRefExpr, removeParens: true) && (declRefExpr.Decl is FunctionDecl functionDecl))
						{
							cursor = functionDecl;
							paramTypes = functionDecl.Parameters.Select((param) => param.Type);
							returnTypeName = this.GetRemappedTypeName(cursor, context: null, functionDecl.ReturnType, out _);
						}
					}

					foreach (var paramType in paramTypes)
					{
						var typeName = this.GetRemappedTypeName(cursor, context: null, paramType, out _);
			...
			{
				name = this.GetTypeName(cursor, context, pointeeType, out nativePointeeTypeName);
				name = this.GetRemappedName(name, cursor, tryRemapOperatorName: false);
				name += '*';
			}
		...
		private void GetTypeSize(Cursor cursor, Type type, ref long alignment32, ref long alignment64, out long size32, out long size64)
		{
			...
			if (type is ArrayType arrayType)
			{
				...
				{
					this.GetTypeSize(cursor, arrayType.ElementType, ref alignment32, ref alignment64, out var elementSize32, out var elementSize64);
			...
			{
				this.GetTypeSize(cursor, attributedType.ModifiedType, ref alignment32, ref alignment64, out size32, out size64);
			}
			else if (type is BuiltinType)
			{
				switch (type.Kind)
				{
					...
					default:
					{
						this.AddDiagnostic(DiagnosticLevel.Error, $"Unsupported builtin type: '{type.TypeClass}.", cursor);
						break;
					}
				}
			}
			...
			{
				this.GetTypeSize(cursor, elaboratedType.NamedType, ref alignment32, ref alignment64, out size32, out size64);
			}
			else if (type is EnumType enumType)
			{
				this.GetTypeSize(cursor, enumType.Decl.IntegerType, ref alignment32, ref alignment64, out size32, out size64);
			}
			...
			else if (type is RecordType recordType)
			{
				...
				if (recordType.Decl is CXXRecordDecl cxxRecordDecl)
				{
					...
					else
					{
						foreach (var baseCXXRecordDecl in cxxRecordDecl.Bases)
						{
							...
							this.GetTypeSize(baseCXXRecordDecl, baseCXXRecordDecl.Type, ref fieldAlignment32, ref fieldAlignment64, out fieldSize32, out fieldSize64);
				...
				foreach (var declaration in recordType.Decl.Decls)
				{
					...
					{
						this.GetTypeSize(fieldDecl, fieldDecl.Type, ref fieldAlignment32, ref fieldAlignment64, out fieldSize32, out fieldSize64);
					...
					{
						this.GetTypeSize(nestedRecordDecl, nestedRecordDecl.TypeForDecl, ref fieldAlignment32, ref fieldAlignment64, out fieldSize32, out fieldSize64);
					}
					else
					{
						continue;
			...
			else if (type is TypedefType typedefType)
			{
				...
				var name = this.GetTypeName(cursor, context: null, type, out _);
				...
				{
					this.GetTypeSize(cursor, typedefType.Decl.UnderlyingType, ref alignment32, ref alignment64, out size32, out size64);
				}
			}
			else
			{
				this.AddDiagnostic(DiagnosticLevel.Error, $"Unsupported type: '{type.TypeClass}'.", cursor);
			}
		}
		...
		private bool HasVtbl(CXXRecordDecl cxxRecordDecl)
		{
			...
			foreach (var cxxBaseSpecifier in cxxRecordDecl.Bases)
			{
				...
				if (this.HasVtbl(baseCxxRecordDecl))
				{
					indirectVtblCount++;
				}
			}
			...
			{
				this.AddDiagnostic(DiagnosticLevel.Warning, "Unsupported cxx record declaration: 'multiple virtual bases'. Generated bindings may be incomplete.", cxxRecordDecl);
			}
		...
		private bool IsExcluded(Cursor cursor) => this.IsExcluded(cursor, out _);
		...
		private bool IsExcluded(Cursor cursor, out bool isExcludedByConflictingDefinition)
		{
			...
			try
			{
				...
				bool IsExcludedByFile(Cursor cursor)
				{
					...
					declLocation.GetFileLocation(out var file, out _, out _, out _);
					...
					declLocation.GetExpansionLocation(out var expansionFile, out var line, out var column, out _);
				...
				bool IsExcludedByName(Cursor cursor, out bool isExcludedByConflictingDefinition)
				{
					...
					if (cursor is NamedDecl namedDecl)
					{
						...
						qualifiedName = this.GetCursorQualifiedName(namedDecl);
						name = this.GetCursorName(namedDecl);
					...
					{
						if (this._config.ExcludeEmptyRecords && IsEmptyRecord(recordDecl))
						{
					...
					{
						if (this._config.ExcludeComProxies && IsComProxy(functionDecl, name))
						{
							isExcludedByConfigOption = true;
						}
						else if (this._config.ExcludeEnumOperators && this.IsEnumOperator(functionDecl, name))
						{
							isExcludedByConfigOption = true;
						}
					...
					if (this._config.ExcludedNames.Contains(qualifiedName))
					{
						if (this._config.LogExclusions)
						{
							...
							this.AddDiagnostic(DiagnosticLevel.Info, message);
						}
						return true;
					}

					if (this._config.ExcludedNames.Contains(name))
					{
						if (this._config.LogExclusions)
						{
					...
					if (isExcludedByConfigOption)
					{
						if (this._config.LogExclusions)
						{
							this.AddDiagnostic(DiagnosticLevel.Info, $"Excluded {kind} '{qualifiedName}' by config option");
						}
						return true;
					}
					...
					if (isExcludedByConflictingDefinition)
					{
						if (this._config.LogExclusions)
						{
							this.AddDiagnostic(DiagnosticLevel.Info, $"Excluded {kind} '{qualifiedName}' by conflicting definition");
						}
						return true;
					}
				...
				bool IsIncludedFileOrLocation(Cursor cursor, CXFile file, CXSourceLocation location)
				{
					...
					if (this._visitedFiles.Add(fileName) && this._config.LogVisitedFiles)
					{
						this.AddDiagnostic(DiagnosticLevel.Info, $"Visiting {fileName} - {location.IsFromMainFile} - {Path.IsPathRooted(fileName)}");
					}

					if (this._config.TraversalNames.Contains(fileName, equalityComparer))
					{
						return true;
					}
					...
					else if ((this._config.TraversalNames.Length == 0) && location.IsFromMainFile)
					{
						return true;
					}
				...
				bool IsComProxy(FunctionDecl functionDecl, string name)
				{
					...
					{
						var typeName = this.GetTypeName(parmVarDecl, context: null, pointerType.PointeeType, out var nativeTypeName);
				...
				{
					var cxxMethodName = this.GetRemappedCursorName(cxxMethodDecl);
					var cxxMethodDeclIndex = -1;

					for (var i = 0; i < cxxRecordDecl.Methods.Count; i++)
					{
						var methodDecl = cxxRecordDecl.Methods[i];
						var methodName = this.GetRemappedCursorName(methodDecl);
				...
				bool IsEmptyRecord(RecordDecl recordDecl)
				{
					...
					{
						if (!this.GetCursorName(recordDecl).EndsWith("__") || (recordDecl.Fields.Count != 1))
						{
							return false;
						}
						...
						if ((this.GetCursorName(field) != "unused") || (field.Type.CanonicalType.Kind != CXTypeKind.CXType_Int))
						{
							return false;
						}
					}
					...
					return !this.TryGetUuid(recordDecl, out _);
				}
			}
			catch
			{
		...
		private bool IsFixedSize(Cursor cursor, Type type)
		{
			...
			{
				return this.IsFixedSize(cursor, attributedType.ModifiedType);
			}
			else if (type is BuiltinType)
			{
				return true;
			...
			{
				return this.IsFixedSize(cursor, elaboratedType.NamedType);
			}
			else if (type is EnumType enumType)
			{
				return this.IsFixedSize(cursor, enumType.Decl.IntegerType);
			}
			...
			else if (type is RecordType recordType)
			{
				...
				return recordDecl.Fields.All((fieldDecl) => this.IsFixedSize(fieldDecl, fieldDecl.Type))
			...
			else if (type is TypedefType typedefType)
			{
				...
				return (remappedName != "IntPtr")
					...
					&& this.IsFixedSize(cursor, typedefType.Decl.UnderlyingType);
			}
			else
			{
				this.AddDiagnostic(DiagnosticLevel.Warning, $"Unsupported type: '{type.TypeClass}'. Assuming unfixed size.", cursor);
				return false;
			}
		}
		...
		private bool IsPrevContextDecl<T>(out T value)
			where T : Decl
		{
			var previousContext = this._context.Last.Previous;
			...
			where T : Stmt
		{
			...
			{
				cursor = this.GetExprAsWritten(expr, removeParens);
			}
		...
		private bool IsUnchecked(string targetTypeName, Stmt stmt)
		{
			switch (stmt.StmtClass)
			{
				...
				case CX_StmtClass.CX_StmtClass_ConditionalOperator:
				{
					...
					return this.IsUnchecked(targetTypeName, conditionalOperator.LHS)
						|| this.IsUnchecked(targetTypeName, conditionalOperator.RHS)
						|| this.IsUnchecked(targetTypeName, conditionalOperator.Handle.Evaluate);
				}
				...
				case CX_StmtClass.CX_StmtClass_BinaryOperator:
				{
					...
					return this.IsUnchecked(targetTypeName, binaryOperator.LHS)
						|| this.IsUnchecked(targetTypeName, binaryOperator.RHS)
						|| this.IsUnchecked(targetTypeName, binaryOperator.Handle.Evaluate)
				...
				case CX_StmtClass.CX_StmtClass_CXXStaticCastExpr:
				{
					...
					var explicitCastExprTypeName = this.GetRemappedTypeName(explicitCastExpr, context: null, explicitCastExpr.Type, out _);

					return this.IsUnchecked(targetTypeName, explicitCastExpr.SubExprAsWritten)
						|| this.IsUnchecked(targetTypeName, explicitCastExpr.Handle.Evaluate)
				...
				case CX_StmtClass.CX_StmtClass_ImplicitCastExpr:
				{
					...
					var implicitCastExprTypeName = this.GetRemappedTypeName(implicitCastExpr, context: null, implicitCastExpr.Type, out _);

					return this.IsUnchecked(targetTypeName, implicitCastExpr.SubExprAsWritten)
						|| this.IsUnchecked(targetTypeName, implicitCastExpr.Handle.Evaluate);
				}
				...
				case CX_StmtClass.CX_StmtClass_DeclRefExpr:
				{
					...
					return (declRefExpr.Decl is VarDecl varDecl) && varDecl.HasInit && this.IsUnchecked(targetTypeName, varDecl.Init);
				}
				...
				case CX_StmtClass.CX_StmtClass_IntegerLiteral:
				{
					...
					return this.IsUnchecked(targetTypeName, signedValue, integerLiteral.IsNegative, isHex: integerLiteral.ValueString.StartsWith("0x"));
				}
				...
				case CX_StmtClass.CX_StmtClass_ParenExpr:
				{
					...
					return this.IsUnchecked(targetTypeName, parenExpr.SubExpr)
						|| this.IsUnchecked(targetTypeName, parenExpr.Handle.Evaluate);
				}
					...
					switch (unaryExprOrTypeTraitExpr.Kind)
					{
						case CX_UnaryExprOrTypeTrait.CX_UETT_SizeOf:
						{
							switch (targetTypeName)
							{
								...
								case "Int16":
								{
									return (size32 != size64) || !this.IsPrevContextDecl<VarDecl>(out _);
								}
				...
				case CX_StmtClass.CX_StmtClass_UnaryOperator:
				{
					...
					return this.IsUnchecked(targetTypeName, unaryOperator.SubExpr)
						|| this.IsUnchecked(targetTypeName, unaryOperator.Handle.Evaluate)
				...
				default:
				{
					this.AddDiagnostic(DiagnosticLevel.Warning, $"Unsupported statement class: '{stmt.StmtClassName}'. Generated bindings may not be unchecked.", stmt);
					return false;
				}
			}
			...
			bool IsOverflow(BinaryOperator binaryOperator)
			{
				...
				if (!this.IsStmtAsWritten<IntegerLiteral>(lhs, out var lhsIntegerLiteral, removeParens: true) || !this.IsStmtAsWritten<IntegerLiteral>(rhs, out var rhsIntegerLiteral, removeParens: true))
				{
					return false;
				}

				var targetTypeName = this.GetRemappedTypeName(binaryOperator, context: null, binaryOperator.Type, out _);
		...
		private bool IsUnchecked(string typeName, CXEvalResult evalResult)
		{
			...
			return this.IsUnchecked(typeName, signedValue, (signedValue < 0), isHex: false);
		}
		...
		private bool IsUnsafe(FieldDecl fieldDecl)
		{
			...
			{
				var name = this.GetTypeName(fieldDecl, context: null, type, out _);
				...
				return this.IsSupportedFixedSizedBufferType(remappedName);
			}

			return this.IsUnsafe(fieldDecl, type);
		}

		private bool IsUnsafe(FunctionDecl functionDecl)
		{
			if (this.IsUnsafe(functionDecl, functionDecl.ReturnType))
			{
				return true;
			}
		...
		private bool IsUnsafe(ParmVarDecl parmVarDecl)
		{
			...
			return this.IsUnsafe(parmVarDecl, type);
		}
		...
		private bool IsUnsafe(RecordDecl recordDecl)
		{
			foreach (var decl in recordDecl.Decls)
			{
				if ((decl is FieldDecl fieldDecl) && this.IsUnsafe(fieldDecl))
				{
					return true;
				}
				else if ((decl is RecordDecl nestedRecordDecl) && nestedRecordDecl.IsAnonymousStructOrUnion && this.IsUnsafe(nestedRecordDecl))
				{
					return true;
				}
			}
			return (recordDecl is CXXRecordDecl cxxRecordDecl) && this.HasVtbl(cxxRecordDecl);
		}
		...
		private bool IsUnsafe(TypedefDecl typedefDecl, FunctionProtoType functionProtoType)
		{
			...
			if (this.IsUnsafe(typedefDecl, returnType))
			{
				return true;
			}
			...
			{
				if (this.IsUnsafe(typedefDecl, paramType))
				{
					return true;
				}
			}
		...
		{
			var name = this.GetTypeName(namedDecl, context: null, type, out _);
		...
		private bool NeedsReturnFixup(CXXMethodDecl cxxMethodDecl)
		{
			...
			if (cxxMethodDecl.IsVirtual)
			{
				...
				switch (canonicalReturnType.TypeClass)
				{
					...
					default:
					{
						this.AddDiagnostic(DiagnosticLevel.Error, $"Unsupported return type for abstract method: '{canonicalReturnType.TypeClass}'. Generated bindings may be incomplete.", cxxMethodDecl);
						break;
					}
				}
			}
		...
		{
			if (this.IsStmtAsWritten<ParenExpr>(stmt, out _))
			{
				this.Visit(stmt);
			}
			else
		...
		private void StartUsingOutputBuilder(string name, bool includeTestOutput = false)
		{
			if (this._outputBuilder != null)
			{
				Debug.Assert(this._outputBuilderUsers >= 1);
				this._outputBuilderUsers++;

				this._outputBuilder.NeedsNewline = true;

				if (includeTestOutput && !string.IsNullOrWhiteSpace(this._config.TestOutputLocation))
				{
			...
			Debug.Assert(this._outputBuilderUsers == 0);

			if (!this._outputBuilderFactory.TryGetOutputBuilder(name, out this._outputBuilder))
			{
				this._outputBuilder = this._outputBuilderFactory.Create(name);
				...
				if (includeTestOutput && !string.IsNullOrWhiteSpace(this._config.TestOutputLocation))
				{
					this._testOutputBuilder = this._outputBuilderFactory.Create($"{name}Tests", isTestOutput: true);

					this._testOutputBuilder.AddUsingDirective("System.Runtime.InteropServices");

					if (this._config.GenerateTestsNUnit)
					{
						this._testOutputBuilder.AddUsingDirective("NUnit.Framework");
					}
					else if (this._config.GenerateTestsXUnit)
					{
						this._testOutputBuilder.AddUsingDirective("Xunit");
					}
				}
			}
			else
			{
				...
				if (includeTestOutput && !string.IsNullOrWhiteSpace(this._config.TestOutputLocation))
				{
					this._testOutputBuilder = this._outputBuilderFactory.GetOutputBuilder($"{name}Tests");
					Debug.Assert(this._testOutputBuilder.IsTestOutput);
					this._testOutputBuilder.NeedsNewline = true;
				}
			}
			this._outputBuilderUsers++;
		...
		{
			if (this._outputBuilderUsers == 1)
			{
				this._outputBuilder = null;
				this._testOutputBuilder = null;
			}
			this._outputBuilderUsers--;
		}
		...
		private bool TryGetUuid(RecordDecl recordDecl, out Guid uuid)
		{
			...
			{
				this.AddDiagnostic(DiagnosticLevel.Warning, $"Multiply uuid attributes for {recordDecl.Name}. Falling back to first attribute.", recordDecl);
			}

			var uuidAttr = uuidAttrs.First();
			var uuidAttrText = this.GetSourceRangeContents(recordDecl.TranslationUnit.Handle, uuidAttr.Extent);
			var uuidText = uuidAttrText.Split(new char[] { '"' }, StringSplitOptions.RemoveEmptyEntries)[1];

			if (!Guid.TryParse(uuidText, out uuid))
			{
				this.AddDiagnostic(DiagnosticLevel.Warning, $"Failed to parse uuid attr text '{uuidAttrText}'. Extracted portion: '{uuidText}'.", recordDecl);
				return false;
			}
			return true;
		}
		...
		private bool TryRemapOperatorName(ref string name, FunctionDecl functionDecl)
		{
			...
			if (functionDecl is CXXConversionDecl)
			{
				...
				var returnTypeName = this.GetRemappedTypeName(cursor: null, context: null, returnType, out _);
		...
		{
			if (this.IsUnchecked(targetTypeName, stmt))
			{
				this._outputBuilder.Write("unchecked");

				var needsCast = this.IsStmtAsWritten<IntegerLiteral>(stmt, out _, removeParens: true) && (stmt.DeclContext is EnumDecl);

				if (this.IsStmtAsWritten<UnaryExprOrTypeTraitExpr>(stmt, out var unaryExprOrTypeTraitExpr, removeParens: true) && ((this.CurrentContext is VarDecl) || this.IsPrevContextDecl<VarDecl>(out _)))
				{
					var argumentType = unaryExprOrTypeTraitExpr.TypeOfArgument;
				...
				{
					this._outputBuilder.Write("((");
					this._outputBuilder.Write(targetTypeName);
					this._outputBuilder.Write(')');
				}

				this.ParenthesizeStmt(stmt);
		...
		{
			var currentContext = this._context.AddLast(cursor);
			...
			{
				this.AddDiagnostic(DiagnosticLevel.Error, $"Unsupported cursor: '{cursor.CursorKindSpelling}'. Generated bindings may be incomplete.", cursor);
			}
		...
		{
			this.Visit(cursors.Except(excludedCursors));
		}

		private void WithAttributes(string remappedName)
		{
			if (this._config.WithAttributes.TryGetValue(remappedName, out var attributes))
			{
				foreach (var attribute in attributes)
				{
					...
					{
						this._outputBuilder.AddUsingDirective("System.ComponentModel");
					}
					...
					this._outputBuilder.Write(attribute);
		...
		{
			if (!this._config.WithLibraryPaths.TryGetValue(remappedName, out var libraryPath) && !this._config.WithLibraryPaths.TryGetValue("*", out libraryPath))
			{
				this._outputBuilder.Write(this._config.LibraryPath);
			}
			else
			{
				this._outputBuilder.Write('"');
				this._outputBuilder.Write(libraryPath);
				this._outputBuilder.Write('"');
			}
		}
		...
		{
			if (this._config.WithSetLastErrors.Contains("*") || this._config.WithSetLastErrors.Contains(remappedName))
			{
				this._outputBuilder.Write(',');
				this._outputBuilder.Write(' ');
				this._outputBuilder.Write("SetLastError");
				this._outputBuilder.Write(' ');
				this._outputBuilder.Write('=');
				this._outputBuilder.Write(' ');
				this._outputBuilder.Write("true");
			}
		}
		...
		private void WithTestAttribute()
		{
			if (this._config.GenerateTestsNUnit)
			{
				this._testOutputBuilder.WriteIndentedLine("[Test]");
			}
			else if (this._config.GenerateTestsXUnit)
			{
				this._testOutputBuilder.WriteIndentedLine("[Fact]");
			}
		}
		...
		private void WithTestAssertEqual(string expected, string actual)
		{
			if (this._config.GenerateTestsNUnit)
			{
				...
				this._testOutputBuilder.Write(actual);
				this._testOutputBuilder.Write(',');
				this._testOutputBuilder.Write(' ');
				this._testOutputBuilder.Write("Is.EqualTo");
				this._testOutputBuilder.Write('(');
				this._testOutputBuilder.Write(expected);
				this._testOutputBuilder.Write(')');
				this._testOutputBuilder.Write(')');
		...
		private void WithTestAssertTrue(string actual)
		{
			if (this._config.GenerateTestsNUnit)
			{
				...
				this._testOutputBuilder.Write(',');
				this._testOutputBuilder.Write(' ');
				this._testOutputBuilder.Write("Is.True");
				this._testOutputBuilder.Write(')');
				this._testOutputBuilder.WriteSemicolon();
				this._testOutputBuilder.WriteNewline();
			}
			...
			{
				this._testOutputBuilder.WriteIndented("Assert.True");
				this._testOutputBuilder.Write('(');
				this._testOutputBuilder.Write(actual);
				this._testOutputBuilder.Write(')');
				this._testOutputBuilder.WriteSemicolon();
		...
		{
			if (this._config.WithTypes.TryGetValue(remappedName, out var type))
			{
				if (string.IsNullOrWhiteSpace(nativeTypeName))
				{
		...
		{
			if (this._config.WithUsings.TryGetValue(remappedName, out var usings))
			{
				foreach (var @using in usings)
				{
					this._outputBuilder.AddUsingDirective(@using);
				}
			}
		}
	}
